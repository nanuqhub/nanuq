MODULE icewri
   ...
   USE sbc_oce , ONLY : ln_blk, qsr, qns ! surf. boundary cond.: ocean
   ...
   USE icedyn_rhg_tools, ONLY : strain_rate_all, sigmaII_full, vel_div_t, vel_shear_f, vel_maxshr_t
...
CONTAINS
   ...
   SUBROUTINE ice_wri( kt )
      ...
      REAL(wp) ::   z2da, z2db, zrho1, zrho2, zA
      ...
      IF( iom_use('iceconc' ) ) THEN
         PRINT *, '    ***** `ice_wri()` activated for GPU at kt =', kt, ' *****'
         !$acc data create( z2d )
         !$acc update self( kmsk_ice_t, kmsk_ice_f, kmsk_ice_u, kmsk_ice_v )
#endif
         ...
         IF( iom_use('icethic' ) ) THEN
            !$acc update self( xht )
            CALL iom_put( 'icethic',   xht * zimskt )   ! ice thickness
         ENDIF
         IF( iom_use('icethic-f' ) ) THEN
            !$acc update self( xhf )
            CALL iom_put( 'icethic-f', xhf * zimskf )   ! ice thickness
         ENDIF

         IF( iom_use('snwthic' ) ) THEN
            !$acc update self(  at_i , vt_s )
            ztmp1(:,:) = 1._wp / MAX( at_i(:,:) , epsi20 )
            ztmp2(:,:) = MERGE( 0._wp  ,  vt_s(:,:) * ztmp1(:,:)  ,  at_i(:,:) <= epsi20 )
            CALL iom_put( 'snwthic', ztmp2 * zimskt )       ! snw thickness
         ENDIF
         ...
         IF( ln_blk ) THEN
            !! Bulk transfer coefficients as seen by the atmosphere: 100% sea-ice => `Cx_ice` | 0% sea-ice => `Cx_oce`
            IF( iom_use("Cd") ) THEN
               !$acc update self( CD_oce, CD_ice )
               CALL iom_put( "Cd"    , ( (1._wp-at_i)*CD_oce + at_i*CD_ice ) * xmskt )
            ENDIF
            IF( iom_use("Ch") ) THEN
               !$acc update self( CH_oce, CH_ice )
               CALL iom_put( "Ch"    , ( (1._wp-at_i)*CH_oce + at_i*CH_ice ) * xmskt )
            ENDIF
            IF( iom_use("Ce") ) THEN
               !$acc update self( CE_oce, CE_ice )
               CALL iom_put( "Ce"    , ( (1._wp-at_i)*CE_oce + at_i*CE_ice ) * xmskt )
            ENDIF
         ENDIF


         IF( ln_rdgtrc ) THEN
            IF( iom_use('rdgc') )  THEN
               !$acc update self( rdgc )
               CALL iom_put( 'rdgc', rdgc * zimskt )   ! fraction of ridged-ice
            ENDIF
         ENDIF
         ...
         IF( iom_use('iceconc-u') ) THEN
            !$acc update self( au_i )
            CALL iom_put( 'iceconc-u', au_i )
         ENDIF
         IF( iom_use('iceconc-v') ) THEN
            !$acc update self( av_i )
            CALL iom_put( 'iceconc-v', av_i )
         ENDIF
         ...
         !
#if ! defined _OPENACC || defined _OPENMP
         CALL lbc_lnk( 'ice_wri', divSx_t,'U',1._wp, divSy_t,'V',1._wp, divSx_f,'V',1._wp, divSy_f,'U',1._wp )
#endif
         !
         IF( iom_use('divSx_t') ) THEN
            !$acc update self( divSx_t )
            CALL iom_put( 'divSx_t', divSx_t )
         ENDIF
         IF( iom_use('divSy_t') ) THEN
            !$acc update self( divSy_t )
            CALL iom_put( 'divSy_t', divSy_t )
         ENDIF
         IF( iom_use('divSx_f') ) THEN
            !$acc update self( divSx_f )
            CALL iom_put( 'divSx_f', divSx_f )
         ENDIF
         IF( iom_use('divSy_f') ) THEN
            !$acc update self( divSy_f )
            CALL iom_put( 'divSy_f', divSy_f )
         ENDIF
         IF( iom_use('ds11t_dx_u') ) THEN
            !$acc update self( ds11t_dx_u )
            CALL iom_put( 'ds11t_dx_u', ds11t_dx_u )
         ENDIF
         IF( iom_use('ds12f_dy_u') ) THEN
            !$acc update self( ds12f_dy_u )
            CALL iom_put( 'ds12f_dy_u', ds12f_dy_u )
         ENDIF
         IF( iom_use('ds22t_dy_v') ) THEN
            !$acc update self( ds22t_dy_v )
            CALL iom_put( 'ds22t_dy_v', ds22t_dy_v )
         ENDIF
         IF( iom_use('ds12f_dx_v') ) THEN
            !$acc update self( ds12f_dx_v )
            CALL iom_put( 'ds12f_dx_v', ds12f_dx_v )
         ENDIF
         IF( iom_use('ds11f_dx_v') ) THEN
            !$acc update self( ds11f_dx_v )
            CALL iom_put( 'ds11f_dx_v', ds11f_dx_v )
         ENDIF
         IF( iom_use('ds12t_dy_v') ) THEN
            !$acc update self( ds12t_dy_v )
            CALL iom_put( 'ds12t_dy_v', ds12t_dy_v )
         ENDIF
         IF( iom_use('ds22f_dy_u') ) THEN
            !$acc update self( ds22f_dy_u )
            CALL iom_put( 'ds22f_dy_u', ds22f_dy_u )
         ENDIF
         IF( iom_use('ds12t_dx_u') ) THEN
            !$acc update self( ds12t_dx_u )
            CALL iom_put( 'ds12t_dx_u', ds12t_dx_u )
         ENDIF
         !
#endif
         ...
         !! Saving stress tensor components and invariants in the proper units! i.e. `Pa`
         !! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
         ...
         lNeed_1oh = ( iom_use('ice_sig11') .OR. iom_use('ice_sig22') .OR. iom_use('ice_sig12') .OR. iom_use('normstr') .OR. iom_use('sheastr')  &
            &     .OR. iom_use('ice_sig11f').OR. iom_use('ice_sig22f').OR. iom_use('ice_sig12t').OR. iom_use('normstrf').OR. iom_use('sheastrf') )

         IF( lNeed_1oh ) THEN
            ALLOCATE( z1_h_t(jpi,jpj), z1_h_f(jpi,jpj) )
            !$acc update self( xht, xhf )
            z1_h_t(:,:) = zimskt(:,:) / MAX( xht(:,:), epsi20 )
            z1_h_f(:,:) = zimskf(:,:) / MAX( xhf(:,:), epsi20 )
         ENDIF

         IF( iom_use('normstr') ) THEN
            !$acc update self( SI1t )
            CALL iom_put( 'normstr',  SI1t(:,:)*z1_h_t(:,:) )
         ENDIF
         IF( iom_use('sheastr') ) THEN
            !$acc update self( SI2t )
            CALL iom_put( 'sheastr',  SI2t(:,:)*z1_h_t(:,:) )
         ENDIF
         IF( iom_use('ice_sig11') .OR. iom_use('ice_sig22') .OR. iom_use('ice_sig12') ) THEN
         ...
         IF( ln_damage ) THEN
            ...
            IF( iom_use('normstrf') ) THEN
               !$acc update self( SI1f )
               CALL iom_put( 'normstrf',  SI1f(:,:)*z1_h_f(:,:) )
            ENDIF
            IF( iom_use('sheastrf') ) THEN
               !$acc update self( SI2f )
               CALL iom_put( 'sheastrf',  SI2f(:,:)*z1_h_f(:,:) )
            ENDIF
            IF( iom_use('ice_sig11f') .OR. iom_use('ice_sig22f') .OR. iom_use('ice_sig12t') ) THEN
         ...
         IF( lNeed_1oh )  DEALLOCATE( z1_h_t, z1_h_f )
         ...
         IF( ln_icethd ) THEN
            ...
            IF( ln_age ) THEN
               IF( iom_use('iceage_cat') ) THEN
                  !$acc update self( a_i, oa_i )
                  DO jl=1, jpl
                     ztmp1(:,:) = 1._wp / ( MAX( a_i(:,:,jl) , epsi20 ) * rday )
                     z3d(:,:,jl) = MERGE( 0._wp  ,  oa_i(:,:,jl) * ztmp1(:,:)  ,  a_i(:,:,jl)<=epsi20 ) * zimskt(:,:)
                  END DO
                  CALL iom_put( 'iceage_cat'  , z3d ) ! ice age
               ENDIF
               !
               IF( iom_use('iceage') ) THEN
                  !$acc update self( at_i, oa_i )
                  CALL iom_put( 'iceage' , MERGE( 0._wp  ,  SUM(oa_i,dim=3) / MAX(at_i,epsi20) / rday * zimskt  ,  at_i<=epsi20 ) )
               ENDIF
            ENDIF
            ...
            IF( iom_use('icesalt' ) ) THEN
               !$acc update self( st_i )
               CALL iom_put( 'icesalt', st_i(:,:) /  MAX(vt_i(:,:),epsi20)  * zimskt(:,:) )
            ENDIF
            IF( iom_use('icesalm' ) )   CALL iom_put( 'icesalm', st_i * rhoi * 1.0e-3 * zimskt )                                  ! Mass of salt in sea ice per cell area

            ! heat
            IF( iom_use('icetemp' ) ) THEN
               ! Mean temperature of ice (old `tm_i`...)
               !$acc data create( ztmp1 )
               !$acc parallel loop collapse(2) present( ztmp1, t_i, v_i )
               DO jj=Njs0-nn_hls, Nje0+nn_hls
                  DO ji=Nis0-nn_hls, Nie0+nn_hls
                     ztmp1(ji,jj) = 0._wp
                     lDo = vt_i(ji,jj) > epsi20
                     !$acc loop seq
                     DO jl = 1, jpl
                        !$acc loop seq
                        DO jk=1, nlay_i
                           ztmp1(ji,jj) = MERGE( ztmp1(ji,jj) + r1_nlay_i * t_i(ji,jj,jk,jl) * v_i(ji,jj,jl) / MAX(vt_i(ji,jj),epsi20)  ,  rt0  ,  lDo )
                        END DO
                     END DO
                  END DO
               END DO
               !$acc end parallel loop
               !$acc update self( ztmp1 )
               !$acc end data
               CALL iom_put( 'icetemp', ( ztmp1  - rt0 ) * zimskt )
            ENDIF
            ...
            IF( iom_use('snwtemp' ) ) THEN
               ! Mean temperature of snow
               !$acc data create( ztmp1 )
               !$acc parallel loop collapse(2) present( ztmp1, t_s, v_s, vt_s )
               DO jj=Njs0-nn_hls, Nje0+nn_hls
                  DO ji=Nis0-nn_hls, Nie0+nn_hls
                     ztmp1(ji,jj) = 0._wp
                     lDo = vt_s(ji,jj) > epsi20
                     !$acc loop seq
                     DO jl = 1, jpl
                        !$acc loop seq
                        DO jk=1, nlay_s
                           ztmp1(ji,jj) = MERGE( ztmp1(ji,jj) + r1_nlay_s * t_s(ji,jj,jk,jl) * v_s(ji,jj,jl) / MAX(vt_s(ji,jj),epsi20)  ,  rt0  ,  lDo )
                        END DO
                     END DO
                  END DO
               END DO
               ...
               !$acc update self( ztmp1 )
               !$acc end data
               CALL iom_put( 'snwtemp', ( ztmp1  - rt0 ) * zimskt )      ! snw mean temperature
            ENDIF


            IF( iom_use('icettop') ) THEN
               !$acc update self(tm_su)
               CALL iom_put( 'icettop', ( tm_su - rt0 ) * zimskt )      ! temperature at the ice surface
            ENDIF
            IF( iom_use('icetbot' ) ) THEN
               !$acc update self(tm_su)
               CALL iom_put( 'icetbot', ( t_bo  - rt0 ) * zimskt )      ! temperature at the ice bottom
            ENDIF
            !IF( iom_use('dtsrfbt') )   CALL iom_put( 'dtsrfbt', ( t_bo-(sst_m+rt0))*zimskt + zmiss_val * ( 1._wp - zimskt ) )      ! `t_bo - sst`


            !IF( iom_use('icetsni' ) ) THEN
            !   ! temperature at the snow-ice interface
            !   !$acc update self( at_i, t_si, a_i )
            !   ztmp1(:,:) = MERGE( 1._wp / MAX( at_i(:,:), epsi20)  ,  0._wp  ,  at_i(:,:) > epsi20 )  ! => 1/A
            !   ztmp2(:,:) = SUM( t_si(:,:,:) * a_i(:,:,:), dim=3  ) * ztmp1(:,:)
            !   CALL iom_put( 'icetsni', ( ztmp2 - rt0 ) * zimskt )
            !ENDIF
            ...
            IF( iom_use('icevolu_cat' ) )   CALL iom_put( 'icevolu_cat' , v_i            * zmskt00l ) ! volume per unit area for categories
            IF( iom_use('icethic_cat' ) )   CALL iom_put( 'icethic_cat' , h_i            * zmskt00l ) ! thickness for categories
            IF( iom_use('snwvolu_cat' ) )   CALL iom_put( 'snwvolu_cat' , v_s            * zmsktsnl ) ! snow depth for categories
            IF( iom_use('snwthic_cat' ) )   CALL iom_put( 'snwthic_cat' , h_s            * zmsktsnl ) ! snow depth for categories
            IF( iom_use('icesalt_cat' ) )   CALL iom_put( 'icesalt_cat' , s_i            * zmskt00l ) ! salinity for categories
            IF( iom_use('icetemp_cat' ) )   CALL iom_put( 'icetemp_cat' , ( SUM( t_i, dim=3 ) * r1_nlay_i - rt0 ) * zmskt00l ) ! ice temperature
            IF( iom_use('snwtemp_cat' ) )   CALL iom_put( 'snwtemp_cat' , ( SUM( t_s, dim=3 ) * r1_nlay_s - rt0 ) * zmsktsnl ) ! snow temperature
            IF( iom_use('icettop_cat' ) )   CALL iom_put( 'icettop_cat' , ( t_su - rt0 ) * zmskt00l  ) ! surface temperature
            !IF( iom_use('icebrv_cat'  ) )   CALL iom_put( 'icebrv_cat'  ,   v_ibr * 100.  * zmskt00l ) ! brine volume
            ...
            !IF( iom_use('icehpnd_cat' ) )   CALL iom_put( 'icehpnd_cat' ,   h_ip         * zmskt00l ) ! melt pond thickness for categories
            !IF( iom_use('icehlid_cat' ) )   CALL iom_put( 'icehlid_cat' ,   h_il         * zmskt00l ) ! melt pond lid thickness for categories
            ...
            !IF( iom_use('icealb_cat'  ) )   CALL iom_put( 'icealb_cat'  ,   alb_ice      * zmskt00l ) ! ice albedo for categories
            ...
            IF(  iom_use('NH_icearea') .OR. iom_use('NH_icevolu') .OR. iom_use('NH_iceextt') .OR. &
               ...
               !
               IF( iom_use('NH_icearea') )   zdiag_area_nh = glob_2Dsum( 'icewri', at_i *           z2d   * e1e2t * 1.e-12 )
               IF( iom_use('NH_icevolu') )   zdiag_volu_nh = glob_2Dsum( 'icewri', vt_i *           z2d   * e1e2t * 1.e-12 )
               IF( iom_use('NH_iceextt') )   zdiag_extt_nh = glob_2Dsum( 'icewri',                  z2d   * e1e2t * 1.e-12 * zimskt )
               !
               IF( iom_use('SH_icearea') )   zdiag_area_sh = glob_2Dsum( 'icewri', at_i * ( 1._wp - z2d ) * e1e2t * 1.e-12 )
               IF( iom_use('SH_icevolu') )   zdiag_volu_sh = glob_2Dsum( 'icewri', vt_i * ( 1._wp - z2d ) * e1e2t * 1.e-12 )
               IF( iom_use('SH_iceextt') )   zdiag_extt_sh = glob_2Dsum( 'icewri',        ( 1._wp - z2d ) * e1e2t * 1.e-12 * zimskt )
               !
   ...
   SUBROUTINE ice_wri_state( kid )
      ...
      !$acc update self( xht, at_i, st_i, u_ice, v_ice, qsr, qns, sf(jp_snow)%fnow(:,:,1), vt_i, h_i, a_i, s_i, h_s )

      CALL iom_rstput( 0, 0, kid, 'sithic', xht          )   ! Ice thickness
      ...
      CALL iom_rstput( 0, 0, kid, 'snwpre', sf(jp_snow)%fnow(:,:,1) )   ! Snow precipitation
      CALL iom_rstput( 0, 0, kid, 'sisali', st_i(:,:) /  MAX(vt_i(:,:),epsi20)         )   ! Ice salinity
