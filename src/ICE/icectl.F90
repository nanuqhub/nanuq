...
CONTAINS
   ...
   !   !!              It prints in nanuq.output if there is a violation of conservation at each time-step
   ...
   !   ztmp3(:,:,2) =   SUM( SUM( szv_i, dim=4 ), dim=3 ) * rhoi                              * e1e2t ! salt
   ...
   !         IF( zchk3(7) > rn_amax+epsi10 .AND. cd_routine /= 'icedyn_adv' .AND. cd_routine /= 'icedyn_rdgrft' ) &
   ...
   !      pdiag_s = SUM( SUM( szv_i, dim=4 ), dim=3 ) * rhoi
   ...
   !      !
   !      ! -- salt diag -- !
   !      zdiag_salt =   ( SUM( SUM( szv_i, dim=4 ), dim=3 ) * rhoi - pdiag_s ) * r1_Dt_ice                                                  &
   ...
   SUBROUTINE ice_ctl( kt )
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpl)  ::   zsv_i
      !!-------------------------------------------------------------------
#if defined _OPENACC || defined _OPENMP
      CALL ctl_stop('STOP', 'ice_ctl() : finish GPU for `ln_icectl=T` !!! (icectl.F90)')
#endif
      !
      zsv_i(:,:,:) = SUM( szv_i(:,:,:,:), dim=3 )
      !
      inb_alp(:) = 0
      ialert_id = 0
         ...
         DO jj=Njs0, Nje0
            DO ji=Nis0, Nie0
               IF( v_i(ji,jj,jl) > epsi10  ) THEN
                  IF( zsv_i(ji,jj,jl) / v_i(ji,jj,jl) > rn_sinew * sss_s(ji,jj) ) THEN
                     WRITE(numout,*) ' PANIC :   Very high salinity ',szv_i(ji,jj,:,jl)/v_i(ji,jj,jl)
      ...
      DO jl = 1, jpl
         DO jk = 1, nlay_i
            DO jj=Njs0, Nje0
               DO ji=Nis0, Nie0
                  IF( v_i(ji,jj,jl) > epsi10  ) THEN
                     IF( szv_i(ji,jj,jk,jl) / v_i(ji,jj,jl) < rn_simin ) THEN
                        WRITE(numout,*) ' ALERTE :   Very low salinity ',szv_i(ji,jj,jk,jl),v_i(ji,jj,jl)
      ...
      DO jj=Njs0, Nje0
         DO ji=Nis0, Nie0
            IF( xmskt(ji,jj) == 0._wp .AND. ( at_i(ji,jj) > 0._wp .OR. vt_i(ji,jj) > 0._wp ) ) THEN
      ...
      DO ji = mi0(ki,nn_hls), mi1(ki,nn_hls)
         DO jj = mj0(kj,nn_hls), mj1(kj,nn_hls)
                  ...
                  IF ( kn == 1 .OR. kn == -1 ) THEN
                     ...
                     WRITE(numout,*) ' masks s,u,v   : ', xmskt(ji,jj), umask(ji,jj,1), vmask(ji,jj,1)
                     ...
                     DO jl = 1, jpl
                        ...
                        WRITE(numout,*) ' szv_i         : ', szv_i(ji,jj,1:nlay_i,jl)
                        WRITE(numout,*)
                     END DO
                  ENDIF
                  ...
                  IF ( kn .EQ. 2 ) THEN
                     ...
                     WRITE(numout,*) ' v_ice(i  ,j)  : ', v_ice(ji,jj)
                     WRITE(numout,*) ' strength      : ', strength(ji,jj)
                     WRITE(numout,*) ' damage@T      : ', 1._wp - dmdt(ji,jj)  !#bbm
                     WRITE(numout,*) ' damage@F      : ', 1._wp - dmdf(ji,jj)  !#bbm
                     WRITE(numout,*)
                     ...
                     DO jl = 1, jpl
                        ...
                        WRITE(numout,*) ' s_i        : ', s_i(ji,jj,jl)              , ' oa_i       : ', oa_i(ji,jj,jl)
                        ...
                        WRITE(numout,*) ' szv_i1     : ', szv_i(ji,jj,1,jl)          , ' szv_i1_b   : ', szv_i_b(ji,jj,1,jl)
                     END DO !jl
                     ...
                     WRITE(numout,*) '   ~~~~~~~~ '
                     WRITE(numout,*) ' taux_ai_t   : ',  taux_ai_t(ji,jj)
                     WRITE(numout,*) ' tauy_ai_t   : ',  tauy_ai_t(ji,jj)
         ...
         SUBROUTINE ice_prt3D( cd_routine )
            ...
            CALL prt_ctl(tab2d_1=SIGMAt(:,:,1), clinfo1=' stress11_i :', mask1=tmask)
            CALL prt_ctl(tab2d_1=SIGMAt(:,:,2), clinfo1=' stress22_i :', mask1=tmask)
            CALL prt_ctl(tab2d_1=SIGMAt(:,:,3), clinfo1=' stress12_i :')   ! should be fmask
            ...
            CALL prt_ctl(tab2d_1=dmdt       , clinfo1= ' dmdt       :', mask1=tmask) !#bbm
            CALL prt_ctl(tab2d_1=dmdf       , clinfo1= ' dmdf       :', mask1=fmask) !#bbm
            ...
            DO jl = 1, jpl
               ...
               DO jk = 1, nlay_i
                  CALL prt_ctl_info(' - Layer : ', ivar=jk)
                  CALL prt_ctl(tab2d_1=szv_i(:,:,jk, jl), clinfo1= ' szv_i   : ', mask1=tmask)
               END DO
            ...
            CALL prt_ctl(tab2d_1=taux_ai_t   , clinfo1= ' taux_ai_t : ', mask1 = umask,  &
               &         tab2d_2=tauy_ai_t   , clinfo2= ' tauy_ai_t : ', mask2 = vmask)
