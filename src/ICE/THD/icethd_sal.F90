...
CONTAINS
   ...
   SUBROUTINE ice_thd_sal(jl_cat, lk_ice_present)
      !!-------------------------------------------------------------------
      !!                ***  ROUTINE ice_thd_sal  ***
      !!
      !! ** Purpose :   computes new salinities in the ice
      !!
      !! ** Method  :   Sice = S(z,t) [Gravity Drainage and Flushing parameterizations]
      !!
      !! ** Case 4 details :
      !!
      ...
      !             !--------------------------------!
      !             ! Gravity Drainage and Flushing  !
      !             !--------------------------------!
      !$acc data present( a_i,dh_i_sum_2d,dh_s_sum_2d,h_i,lk_ice_present,sfx_bri,sfx_res,sss_s,sz_i,t_i,t_su,t_bo ) create( zds,z_mid,z_edge,zv_br,zRa,zperm_eff,zw_br,zmsk,zs_br )
      !
      ! ==============
      ! Initialization
      ! ==============
         ...
         DO jj=Njs0, Nje0
            DO ji=Nis0, Nie0
               IF( lk_ice_present(ji,jj) ) THEN
                  ...
                  !$acc loop seq
                  DO jk = 1, nlay_i
                     zds(jk) =           MAX( 0._wp, rn_simin                - sz_i(ji,jj,jk,jl_cat) ) ! > 0 if sz_i < simin
                     zds(jk) = zds(jk) + MIN( 0._wp, rn_sinew * sss_s(ji,jj) - sz_i(ji,jj,jk,jl_cat) ) ! < 0 if sz_i > simax
                     ! update salinity
                     sz_i(ji,jj,jk,jl_cat) = sz_i(ji,jj,jk,jl_cat) + zds(jk)
                     ! salt flux
   ...
   SUBROUTINE ice_thd_sal_init
      ...
      !!
      NAMELIST/namthd_sal/ ln_flushing, ln_drainage, rn_sinew, rn_simin, &
      ...
      ENDIF
      !
      !$acc update device( rn_sinew,rn_simin,ln_sal_chk,nn_liquidus,nn_sal_scheme,ln_flushing )
