MODULE icealb
   ...
   USE par_ice, ONLY: jpl, rcloud_fra ! sea-ice: number of categories
   ...
   PUBLIC   ice_alb_pnd    ! called in icesbc.F90 and iceupdate.F90
   ...
   REAL(wp), PUBLIC, SAVE ::   rn_alb_sdry      ! dry snow albedo
   REAL(wp), PUBLIC, SAVE ::   rn_alb_smlt      ! melting snow albedo
   REAL(wp), PUBLIC, SAVE ::   rn_alb_idry      ! dry ice albedo
   REAL(wp), PUBLIC, SAVE ::   rn_alb_imlt      ! bare puddled ice albedo
...
CONTAINS
   ...
   SUBROUTINE ice_alb_pnd( pt_su, ph_ice, ph_snw, pafrac_pnd, ph_pnd, palb_ice )
      !!----------------------------------------------------------------------
      !!               ***  ROUTINE ice_alb_pnd  ***
      !!
      ...
      !!---------------------------------------------------------------------
      IF( ln_timing )   CALL timing_start('ice_alb_pnd')
      !$acc data present( pt_su, ph_ice, ph_snw, pafrac_pnd, ph_pnd, palb_ice )
      !
      ...
      DO jl = 1, jpl
         DO jj=Njs0-1, Nje0+1
            ...
            DO ji=Nis0-1, Nie0+1
               ...
               zalb_os = ( zafrac_snw * zalb_snw + zafrac_pnd * zalb_pnd + zafrac_ice * zalb_ice ) * xmskt(ji,jj)
               !
   ...
   SUBROUTINE ice_alb( pt_su, ph_ice, ph_snw, palb_ice )
      !!----------------------------------------------------------------------
      !!               ***  ROUTINE ice_alb  ***
      !!
      !! ** Purpose :   Computation of the albedo of the snow/ice system
      ...
      !LOGICAL  :: ldbg
      !!---------------------------------------s------------------------------
      IF( ln_timing )   CALL timing_start('ice_alb')
      !$acc data present( pt_su, ph_ice, ph_snw, palb_ice )
      !
      z1_c1 = 1._wp / ( LOG(rn_alb_hpiv) - LOG(0.05_wp) )
      z1_c2 = 1._wp / 0.05_wp
      z1_c3 = 1._wp / 0.02_wp
      ...
      !
      !$acc end data
      IF( ln_timing )   CALL timing_stop('ice_alb')
      !
