MODULE icedyn_rhg_meb
   ...
   !!----------------------------------------------------------------------
   !!----------------------------------------------------------------------
   !!   ice_dyn_rhg_meb : computes ice velocities from MEB rheology
   !!----------------------------------------------------------------------
   ...
   PUBLIC   ice_dyn_rhg_meb_init  ! called by icedyn_rhg.F90
   PUBLIC   ice_dyn_rhg_meb       ! called by icedyn_rhg.F90
   ...
   CHARACTER(len=15), PARAMETER :: crtnm = 'ice_dyn_rhg_meb'
...
CONTAINS
   ...
   SUBROUTINE ice_dyn_rhg_meb( kt, pshear_i, pdivu_i, pdelta_i )
      !!-------------------------------------------------------------------
      !!                 ***  SUBROUTINE ice_dyn_rhg_meb  ***
      !!                             MEB-E-grid
      !!
      ...
      !!  the MEB rheology of Dansereau et al., 2016.
      !!
      ...
      IF( kt == nit000 .AND. lwp )   WRITE(numout,*) '-- '//crtnm//': MEB sea-ice rheology'
      ...
      !!       `xxpCt` as wells as their F-point counterparts
      ...
      DO jter = 1 , nbrttl                   !    loop over jter    !
         ...
         CALL update_sigma_d( kt, jter, rdtbri, V_ts, at_i, af_i, xxpCt, xxpCf, xScHt, xScHf, &
      ...
      !
   END SUBROUTINE ice_dyn_rhg_meb


   SUBROUTINE ice_dyn_rhg_meb_init( )
      !!-------------------------------------------------------------------
      ...
      !$acc enter data copyin( xxpCt, xxpCf, xgrdH, xScHt, xScHf )
      PRINT *, '    ==> xxpCt, xxpCf, xgrdH, xScHt, xScHf'
#endif

   END SUBROUTINE ice_dyn_rhg_meb_init


   SUBROUTINE update_sigma_d( kt, kts, pdt, pV4, pAt, pAf, pxpCt, pxpCf, pScHt, pScHf, &
      ...
      REAL(wp) :: zxpC, z1md, zsigI, zsigII
      REAL(wp) :: z1_sigI, zecc, zCohe
      REAL(wp) :: zrr
      !!----------------------------------------------------------------------
      REAL(wp)               :: zxpCt, zScHt, zht, z1mdt, zmskt
      REAL(wp)               :: zxpCf, zScHf, zhf, z1mdf, zmskf
...
#              include "icedyn_rhg_bri_strn_t.h90"
               ...
               !     ********************************************
#              include "icedyn_rhg_meb_elm_t.h90"
               ! ==> zEt, zLt, zmult
...
#              include "icedyn_rhg_bri_strn_f.h90"
               ...
               ! --- E, Lambda & multiplicator (uses `sigmas`!!!) ---
               !     ********************************************
#              include "icedyn_rhg_meb_elm_f.h90"
               ! ==> zEf, zLf, zmulf
