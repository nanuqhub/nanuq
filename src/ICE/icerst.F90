...
CONTAINS
   ...
   !   CHARACTER(LEN=128)  ::   clname   ! ocean output restart file name
   ...
   SUBROUTINE ice_rst_opn( kt )
      ...
      CHARACTER(len=128)  ::   clname   ! ice output restart file name
      CHARACTER(len=256)  ::   clpath   ! full path to ice output restart file
      CHARACTER(LEN=52)   ::   clpname   ! ocean output restart file name including prefix for AGRIF
      !!----------------------------------------------------------------------
      !
      IF( kt == nit000 ) THEN
         lrst_ice = .FALSE.   ! default definition
         !
         IF( ln_rst_list ) THEN
            nrst_lst = 1
            nitrst = nn_stocklist( nrst_lst )
...
#else
                  CALL ctl_stop( 'Can not use XIOS in `ice_rst_opn`' )
#endif
               ENDIF
               lrst_ice = .TRUE.
            ENDIF
   ...
   SUBROUTINE ice_rst_read()
      ...
      ELSE                 ! == case of a simplified restart == !
         ...
         ENDIF
         !
         IF( ln_cpl_oce ) THEN   ! SAS case: ss[st]_m were not initialized by oss_prs_init
            ...
            !sst_m(:,:) = ts(:,:,1,jp_tem, Kmm) !LOLO_SI3SA
            !sss_m(:,:) = ts(:,:,1,jp_sal, Kmm)
            CALL ctl_stop('STOP', 'ice_rst_read: LOLO why are we here?')
         ENDIF
         !
