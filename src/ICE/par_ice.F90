MODULE par_ice
   ...
   REAL(wp), PARAMETER, PUBLIC :: rAmin_vel  = 0.001_wp !: ice concentration below which ice velocity becomes very small
   REAL(wp), PARAMETER, PUBLIC :: rMmin_vel  = 1._wp    !: ice mass (kg/m2)  below which ice velocity becomes very small
   REAL(wp), PARAMETER, PUBLIC :: rAmin_fld  = 0.01_wp  !: ice concentration below which we hide relevant fields in output netCDF files...
   REAL(wp), PARAMETER, PUBLIC :: rAmin_dmg  = 0.1_wp   !: ice concentration below which damage becomes a nonsense and is forced to 0
   REAL(wp), PARAMETER, PUBLIC :: rAmin_sgmh = 0.1_wp   !: ice concentration below which `sigma*h` stresses becomes a nonsense and are forced to 0
   !REAL(wp), PARAMETER, PUBLIC :: rAmin_divS = 0.05_wp !: ice concentration below which horizontal gradient (finite dif) has to be canceled!
   REAL(wp), PARAMETER, PUBLIC :: rAmin_lbc  = 0.1_wp   !: ice concentration below which horizontal gradient (finite dif) of `sigma*h` needs a LBC !
   ...
   INTEGER           , PUBLIC ::   nn_jpl           !: number of ice  categories as read in namelist_ice
   INTEGER           , PUBLIC ::   nn_nlay_i        !: number of ice  layers     "             "
   INTEGER           , PUBLIC ::   nn_nlay_s        !: number of snow layers     "             "

#if defined _OPENACC || defined _OPENMP
   !! Must be hardcoded for GPU:
   INTEGER,  PARAMETER, PUBLIC ::  jpl    = 5       !: number of ice  categories
   INTEGER,  PARAMETER, PUBLIC ::  nlay_i = 2       !: number of ice  layers
   INTEGER,  PARAMETER, PUBLIC ::  nlay_s = 2       !: number of snow layers
   REAL(wp), PARAMETER, PUBLIC ::  r1_nlay_i = 1._wp/nlay_i  !: 1 / nlay_i
   REAL(wp), PARAMETER, PUBLIC ::  r1_nlay_s = 1._wp/nlay_s  !: 1 / nlay_s
#else
   ...
   !$acc declare create( jpl, nlay_i, nlay_s, r1_nlay_i, r1_nlay_s )
#endif
   ...
   LOGICAL           , PUBLIC ::   ln_damage        !: existence of the "ice damage" tracer in the code? => (T) if `ln_rhg_brittle==.true.`
   LOGICAL           , PUBLIC ::   ln_age           !: do we care about the "age of the ice" tracer or not ?
   LOGICAL           , PUBLIC ::   ln_rdgtrc        !: do we want the "ridged-ice" concentration tracer to be active ?
   !$acc declare create( rn_amax )
   ...
   !
   LOGICAL, PUBLIC, SAVE :: ln_bri_rk3    ! Use a RK3 implementation for the time integration of the momentum equation
   !                                      !       => when `false`, use the traditional 1st-order Euler scheme
   !                                      !          (implicit in terms of ice velocity for the bottom ice-water drag though)
   INTEGER, PUBLIC, SAVE :: nn_rk3_iter_v !  number of iterations for convergence of ice-oce stress term `rho_o*CD_o*||U_oce-U_ice||/mass`
   !                                      !       => 1 itteration is okay for most applications        => faster
   !                                      !       => 4 iterations and it is generally fully converged  => costly
   !$acc declare create( nn_rk3_iter_v )
   ...
   LOGICAL,  PUBLIC ::   ln_idealized     !: flag for "idealized test-case": .true.=> Coriolis and SSH tilt terms will be disregarded in the momentum eq.
   REAL(wp), PUBLIC ::   ridlzd

   ! -- "dummy" rheology
   LOGICAL , PUBLIC ::   ln_rhg_DUMMY     ! DUMMY rheology => No rheology, sea-ice velocity is set to that of the surface of the ocean
   ...
   !
   ! -- Brittle rheologies
   LOGICAL , PUBLIC ::   ln_rhg_brittle   ! BRITTLE rheology
   REAL(wp), PUBLIC ::   rn_cfl_coeff     !  factor used for the CFL test (w.r.t. elastic shear wave propagation speed) to define the small rheology time-step
   !                                      !      => the smaller the smaller the small time step, i.e. the larger `nbrttl`
   LOGICAL , PUBLIC ::   ln_rhg_MEB       ! MEB brittle rheology
   LOGICAL , PUBLIC ::   ln_rhg_BBM       ! BBM brittle rheology
   INTEGER , PUBLIC ::   nbrttl           !: number of iterations for subcycling !
   REAL(wp), PUBLIC ::   rn_N_ref          !: Maximum compressive stress at the reference scale [Pa] / neXtSIM => `compr_strength`
   ...
   LOGICAL,  PUBLIC ::   ln_MCx_test     !: Perform only 1 Mohr-Coulomb test, at mid-point between T & F points (implicit smoothing)
   REAL(wp), PUBLIC ::   rn_crndg         !: cross-nudging coefficient ...
   !$acc declare create( ln_rhg_brittle, rn_cfl_coeff, ln_rhg_MEB, ln_rhg_BBM, nbrttl, rn_N_ref, rn_P0, rn_E0, rn_eta0, rn_kth, nn_d_adv, rn_crndg )
   !
   ...
   !
   REAL(wp), PUBLIC ::   rn_pow_scl_res   !: "power" at which the cohesion scales w.r.t local grid resolution / reference scale
   ...
   !$acc declare create( rn_sinew,rn_simin,ln_sal_chk,nn_liquidus,nn_sal_scheme,ln_flushing )
   ...
   !$acc declare create( rDt_ice, r1_Dt_ice )
