...
#  undef OPERATION_MAXLOC
   ...
   SUBROUTINE mpp_ini_znl( kumout )
      ...
      !!
      !!----------------------------------------------------------------------
      INTEGER, INTENT(in) ::   kumout   ! nanuq.output logical units
      !
   ...
   SUBROUTINE ctl_stop( cd1, cd2, cd3, cd4, cd5 ,   &
      ...
      nstop = nstop + 1
      !
      IF( cd1 == 'STOP' .AND. narea /= 1 ) THEN    ! Immediate stop: add an arror message in 'nanuq.output' file
         CALL ctl_opn( inum, 'nanuq.output', 'APPEND', 'FORMATTED', 'SEQUENTIAL', -1, 6, .FALSE. )
         WRITE(inum,*)
         WRITE(inum,*) ' ==>>>   Look for "E R R O R" messages in all existing *nanuq.output* files'
         CLOSE(inum)
      ENDIF
      IF( numout == 6 ) THEN                       ! force to open nanuq.output file if not already opened
