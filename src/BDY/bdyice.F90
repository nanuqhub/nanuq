...
CONTAINS
   ...
   SUBROUTINE bdy_ice( kt )
      ...
      IF( ANY(llsend1) .OR. ANY(llrecv1) ) THEN   ! if need to send/recv in at least one direction
         ...
         CALL lbc_lnk('bdyice', a_i , 'T', 1._wp, h_i , 'T', 1._wp, h_s , 'T', 1._wp, oa_i, 'T', 1._wp  &
            &                 , s_i , 'T', 1._wp, t_su, 'T', 1._wp, v_i , 'T', 1._wp, v_s , 'T', 1._wp  &
   ...
   SUBROUTINE bdy_ice_frs( idx, dta, kt, jbdy, llrim0 )
      ...
      REAL(wp) ::   ztmelts, zdh, zflgu, zflgv
      !!------------------------------------------------------------------------------
      IF( ln_timing )   CALL timing_start('bdy_ice_frs')
      !$acc data present( a_i, h_i, h_s, t_i, t_s, t_su, s_i, sz_i, oa_i, hi_max, u_ice, v_ice, idx, dta, idx%nbi, idx%nbj, idx%nbw, idx%flagu, idx%flagv, dta%a_i, dta%h_i, dta%h_s, dta%tsu, dta%s_i )

      jgrd = 1      ! Everything is at T-points here

      ibeg = MERGE( 1  ,  idx%nblenrim0(jgrd)+1                 ,  llrim0 )
      iend = MERGE( idx%nblenrim0(jgrd)  ,  idx%nblenrim(jgrd)  ,  llrim0 )
      ...
      DO i_bdy = ibeg, iend
         ...
         DO jl = 1, jpl
            ...
            ji = idx%nbi(i_bdy,jgrd)
            jj = idx%nbj(i_bdy,jgrd)
            zflgu = idx%flagu(i_bdy,jgrd)
            zflgv = idx%flagv(i_bdy,jgrd)
            ...
            IF( nn_ice_dta(jbdy) == 0 ) THEN
               !#LOLObug: following line used to be right after the I() as a 1 liner ! => wrong!? MAKES AS DIFFERENCE!!!
               jpbound = 0 ; ib = ji ; jb = jj   ! case ice boundaries = initial conditions
            ...
            IF( a_i(ib,jb,jl) > 0._wp ) THEN   ! there is ice at the boundary
               ...
               !$acc loop seq
               DO jk = 1, nlay_i
                  szv_i(ji,jj,jk,jl) = MIN( sz_i(ji,jj,jk,jl) , sss_m(ji,jj) ) * v_i(ji,jj,jl) ! salt content
   ...
   SUBROUTINE bdy_ice_dyn( cgt, pvel,  l_FcVel )
      !!------------------------------------------------------------------------------
      ...
      !!
      !!------------------------------------------------------------------------------
      CHARACTER(len=1),             INTENT(in)    ::   cgt       ! nature of velocity grid point ('U' or 'V')
      ...
      !$acc data present( pvel, idx_bdy, idx_bdy(1)%nblenrim0, idx_bdy(1)%nblenrim, idx_bdy(1)%nbi, idx_bdy(1)%nbj, idx_bdy(1)%flagu, idx_bdy(1)%flagv, nn_ice_dta(1) )

      lFcV = .FALSE.
      IF(PRESENT(l_FcVel)) lFcV = l_FcVel

      !! Index to access the relevant ocean velocity component (V_oce(:,:,1->4)):
      ixU = MERGE( 1  ,  4  ,  .NOT. lFcV )
      ixV = MERGE( 2  ,  3  ,  .NOT. lFcV )
#if ! defined _OPENACC || defined _OPENMP
         ...
         DO jbdy = 1, nb_bdy
            ...
            CASE('frs')
               ...
               CASE ( 'U' )
                  !
                  jgrd = 2      ! u velocity
                  ibeg = MERGE( 1                              ,  idx_bdy(jbdy)%nblenrim0(jgrd)+1  , ir == 0 )
                  iend = MERGE( idx_bdy(jbdy)%nblenrim0(jgrd)  ,  idx_bdy(jbdy)%nblenrim(jgrd)     , ir == 0 )
                  !
                  !$acc parallel loop
                  DO i_bdy = ibeg, iend
                     !
   ...
   SUBROUTINE bdy_ice_dmg_frs( idx, dta, kt, kts, jbdy, llrim0, pV )
      ...
      REAL(wp) ::   zwgt, zwgt1, zflgu, zflgv
      !!------------------------------------------------------------------------------
      IF(lwp .AND. (kts==10)) WRITE(numout,*) ' * doing `bdy_ice_dmg_frs` for kt, kts',kt,kts
      !$acc data present( dmdt, pV, idx, dta, idx%nbi, idx%nbj, idx%nbw, dta%dmg )
   ...
   SUBROUTINE bdy_dmg_neumann( idx, dta, kt, kts, ir, jbdy )
...
#endif
      !!------------------------------------------------------------------------------
      !$acc data present( dmdt, idx, dta, idx%nbi, idx%nbj, idx%nbw, dta%dmg )
      ...
      DO i_bdy = ibeg, iend
         ...
         IF( zflagu == -1. .AND. ji > 1 .AND. ji < jpi )   THEN
            IF    ( vt_i(ji  ,jj) > 0. )   THEN
#if defined key_verbose
               IF(ldebug) PRINT *, '* DEBUG_BDY: dmdt at ji=',INT(ji-nn_hls,2),' takes value from ji=',INT(ji-nn_hls-1,2)
#endif
               dmdt(ji,jj) = dmdt(ji-1,jj)
               dmdf(ji,jj) = dmdt(ji-1,jj) !lolo?
            ELSEIF( vt_i(ji+1,jj) > 0. )   THEN
#if defined key_verbose
               IF(ldebug) PRINT *, '* DEBUG_BDY: dmdt at jj=',INT(jj-nn_hls,2),' takes `1.` value'
#endif
         ...
         IF( zflagv == -1. .AND. jj > 1 .AND. jj < jpj )   THEN
            IF    ( vt_i(ji,jj  ) > 0. )   THEN
#if defined key_verbose
               IF(ldebug) PRINT *, '* DEBUG_BDY: dmdt at jj=',INT(jj-nn_hls,2),' takes value from jj=',INT(jj-nn_hls-1,2)
#endif
