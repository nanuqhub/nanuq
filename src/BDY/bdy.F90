MODULE bdy
   ...
   TYPE, PUBLIC ::   OBC_INDEX    !: Indices and weights which define the open boundary
      ...
      INTEGER , ALLOCATABLE, DIMENSION(:,:)    ::  nbj
      INTEGER , ALLOCATABLE, DIMENSION(:,:)    ::  nbr
      INTEGER , ALLOCATABLE, DIMENSION(:,:)    ::  nbmap
      INTEGER , ALLOCATABLE, DIMENSION(:,:)    ::  ntreat
      REAL(wp), ALLOCATABLE, DIMENSION(:,:)    ::  nbw
      REAL(wp), ALLOCATABLE, DIMENSION(:,:)    ::  nbd
      REAL(wp), ALLOCATABLE, DIMENSION(:,:)    ::  nbdout
      REAL(wp), ALLOCATABLE, DIMENSION(:,:)    ::  flagu
      REAL(wp), ALLOCATABLE, DIMENSION(:,:)    ::  flagv
   END TYPE OBC_INDEX
   ...
   TYPE, PUBLIC ::   OBC_DATA     !: Storage for external data
      ...
      LOGICAL                           ::  lneed_ice
      LOGICAL                           ::  lneed_dmg
      REAL(wp), ALLOCATABLE, DIMENSION(:,:) ::  a_i    !: now ice leads fraction climatology
      REAL(wp), ALLOCATABLE, DIMENSION(:,:) ::  h_i    !: Now ice  thickness climatology
      REAL(wp), ALLOCATABLE, DIMENSION(:,:) ::  h_s    !: now snow thickness
      REAL(wp), ALLOCATABLE, DIMENSION(:,:) ::  t_i    !: now ice  temperature
      REAL(wp), ALLOCATABLE, DIMENSION(:,:) ::  t_s    !: now snow temperature
      REAL(wp), ALLOCATABLE, DIMENSION(:,:) ::  tsu    !: now surf temperature
      REAL(wp), ALLOCATABLE, DIMENSION(:,:) ::  s_i    !: now ice  salinity
      REAL(wp), ALLOCATABLE, DIMENSION(:)   ::  dmg    !: now ice  damage
      REAL(wp), ALLOCATABLE, DIMENSION(:,:) ::  aip    !: now ice  pond concentration
      REAL(wp), ALLOCATABLE, DIMENSION(:,:) ::  hip    !: now ice  pond depth
      REAL(wp), ALLOCATABLE, DIMENSION(:,:) ::  hil    !: now ice  pond lid depth
   END TYPE OBC_DATA
   ...
   !: = 1 read it in a NetCDF file
   CHARACTER(len=20), DIMENSION(jp_bdy) ::   cn_dmg         ! Choice of boundary condition for damage
   INTEGER , DIMENSION(jp_bdy)          ::   nn_dmg_dta     !: = 0 use the initial state as bdy dta ;
   !: = 1 read it in a NetCDF file
   !
   !                                                   !!** nambdy_dta **
   ...
   REAL(wp), DIMENSION(jp_bdy) ::   rice_dmg                !: damage      of incoming sea ice
   ...
   TYPE(OBC_INDEX), DIMENSION(jp_bdy)      ::   idx_bdy           !: bdy indices (local process)
...
CONTAINS
   ...
   FUNCTION bdy_alloc()
      ...
      bdyvmask(:,:) = 1._wp
      !
      CALL mpp_sum ( 'bdy', bdy_alloc )
      IF( bdy_alloc /= 0 )   CALL ctl_stop( 'STOP', 'bdy_alloc: failed to allocate arrays.' )
      !
   END FUNCTION bdy_alloc


   SUBROUTINE bdy_dealloc()
      IF( ALLOCATED(bdytmask) )   DEALLOCATE( bdytmask, bdyumask, bdyvmask )
   END SUBROUTINE bdy_dealloc
