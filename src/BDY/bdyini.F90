MODULE bdyini
   ...
   USE bdy        ! unstructured open boundary conditions
   ...
   PUBLIC   bdy_init    ! routine called in nanuq_init
...
CONTAINS
   ...
   SUBROUTINE bdy_init
      ...
      !!----------------------------------------------------------------------
      NAMELIST/nambdy/ ln_bdy, nb_bdy, ln_coords_file, cn_coords_file, ln_mask_file, cn_mask_file, &
         &             rn_time_dmp, rn_time_dmp_out, cn_ice, nn_ice_dta, cn_dmg, nn_dmg_dta,       &
         &             nn_rimwidth
      !
      !
      ...
      cn_dmg         (2:jp_bdy) = cn_dmg         (1)
      nn_dmg_dta     (2:jp_bdy) = nn_dmg_dta     (1)
   ...
   SUBROUTINE bdy_def
      ...
      DO ib_bdy = 1,nb_bdy
         ...
         IF(lwp) THEN
            ...
            END SELECT
            !
            WRITE(numout,*) 'Boundary conditions for damage:  '
            SELECT CASE( cn_dmg(ib_bdy) )
            CASE('none')   ;   WRITE(numout,*) '      no open boundary condition'
            CASE('frs')    ;   WRITE(numout,*) '      Flow Relaxation Scheme'
            CASE('neumann');   WRITE(numout,*) '      Flow Relaxation Scheme'
            CASE DEFAULT   ;   CALL ctl_stop( 'unrecognised value for cn_dmg' )
            END SELECT
         ENDIF
         ...
         dta_bdy(ib_bdy)%lneed_dmg = cn_dmg(ib_bdy) /= 'none'

         IF( lwp .AND. dta_bdy(ib_bdy)%lneed_dmg ) THEN
            SELECT CASE( nn_dmg_dta(ib_bdy) )                   !
            CASE( 0 )      ;   WRITE(numout,*) '      initial state used for bdy damage'
            CASE( 1 )      ;   WRITE(numout,*) '      boundary data taken from file'
      ...
      DO ib_bdy = 1, nb_bdy
         DO igrd = 1, jpbgrd
            DO ib = 1, idx_bdy(ib_bdy)%nblenrim(igrd)   ! only the rim triggers communications, see bdy routines
               ...
               IF( ib <= idx_bdy(ib_bdy)%nblenrim0(igrd) ) THEN
                  ir = 0
               ELSE
                  ir = 1
               END IF
