...
CONTAINS
   ...
   SUBROUTINE bdy_frs( idx, phia, dta )
      ...
      REAL(wp), DIMENSION(:,:), ALLOCATABLE,   INTENT(in) ::   dta  ! OBC external data
   ...
   SUBROUTINE bdy_orlanski_2d( idx, igrd, phib, phia, phi_ext, lrim0, ll_npo )
      ...
      REAL(wp), DIMENSION(:  ), ALLOCATABLE, INTENT(in   ) ::   phi_ext  ! external forcing data
      ...
      REAL(wp), DIMENSION(jpi,jpj)          :: zmask      ! land/sea mask for field
      REAL(wp), DIMENSION(jpi,jpj)          :: zmask_xdif ! land/sea mask for x-derivatives
      REAL(wp), DIMENSION(jpi,jpj)          :: zmask_ydif ! land/sea mask for y-derivatives
      REAL(wp), DIMENSION(jpi,jpj)          :: pe_xdif    ! scale factors for x-derivatives
      REAL(wp), DIMENSION(jpi,jpj)          :: pe_ydif    ! scale factors for y-derivatives
      !!----------------------------------------------------------------------
      !
      ! ----------------------------------!
      ...
      IF( lrim0 ) THEN
         ibeg = 1
         iend = idx%nblenrim0(igrd) ! rim 0
      ELSE
         ibeg = idx%nblenrim0(igrd)+1
         iend = idx%nblenrim(igrd) ! rim 1
      ENDIF
      !
      DO jb = ibeg, iend
         ii  = idx%nbi(jb,igrd)
