MODULE diawri
   ...
   PUBLIC   dia_wri_alloc           ! Called by nanuqgcm module
#if ! defined key_xios
...
# if defined key_xios
   ...
   SUBROUTINE dia_wri( kt, Kmm )
      ...
      ELSE
         clop = "x"         ! no use of the mask value (require less cpu time)
      ENDIF
      ...
      IF( kt == nit000 ) THEN
         ...
         CALL histdef( nid_T, "sss_m", "Sea Surface Salinity"               , "PSU"    ,   &  ! sss
            &          jpi, jpj, nh_T, 1  , 1, 1  , -99 , 32, clop, zsto, zout )
         CALL histdef( nid_T, "sowaflup", "Net Upward Water Flux"              , "Kg/m2/s",   &  ! (E-P, no runoffs in NANUQ!)
            &          jpi, jpj, nh_T, 1  , 1, 1  , -99 , 32, clop, zsto, zout )
         ...
         CALL histdef( nid_T, "soshfldo", "Shortwave Radiation"                , "W/m2"   ,   &  ! qsr
            &          jpi, jpj, nh_T, 1  , 1, 1  , -99 , 32, clop, zsto, zout )
         CALL histdef( nid_T, "soicecov", "Ice fraction"                       , "[0,1]"  ,   &  ! at_i
            &          jpi, jpj, nh_T, 1  , 1, 1  , -99 , 32, clop, zsto, zout )
      ...
      CALL histwrite( nid_T, "sowaflup", it, emp  , ndim_hT, ndex_hT ) !E-P, no runoffs in NANUQ!
      ...
      CALL histwrite( nid_T, "soicecov", it, at_i          , ndim_hT, ndex_hT )   ! ice fraction
   ...
   SUBROUTINE dia_wri_state( Kmm, cdfile_name )
      ...
      !CALL iom_rstput( 0, 0, inum, 'sossheig', ssh(:,:,         Kmm) )    ! sea surface height  !#lolo?
      ...
      CALL iom_rstput( 0, 0, inum, 'sowaflup', emp                   )    ! freshwater budget (E-P, no runoffs in NANUQ!)
      CALL iom_rstput( 0, 0, inum, 'sohefldo', qsr + qns             )    ! total heat flux
      CALL iom_rstput( 0, 0, inum, 'soshfldo', qsr                   )    ! solar heat flux
      CALL iom_rstput( 0, 0, inum, 'soicecov', at_i                  )    ! ice fraction
