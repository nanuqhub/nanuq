...
CONTAINS
   ...
   FUNCTION pres_temp_sclr( pqspe, pslp, pz, ptpot, pta, l_ice )
      ...
      INTEGER                           :: it                ! iteration indice and number
      ...
      DO it = 1, niter
         ...
         zpa   = pslp * EXP( -grav * zxm * pz / ( R_gas * MAX(zta,1._wp ) ) )
      END DO
   ...
   FUNCTION theta_exner_sclr( pta, ppa )
      ...
      REAL(wp)             :: ztp
      !
      ztp = MAX(pta, 100._wp) * ( rpref / MAX(ppa, 1.E4_wp) ) ** rgamma_dry
      !
      theta_exner_sclr = MIN( MAX( ztp, 100._wp ) , 350._wp )
      !
   ...
   FUNCTION theta_exner_vctr( pta, ppa )
      ...
      DO jj=Njs0-1, Nje0+1
         DO ji=Nis0-1, Nie0+1
            z2d(ji,jj) = theta_exner_sclr( pta(ji,jj), ppa(ji,jj) )
         END DO
      END DO
      !
      theta_exner_vctr(:,:) = MIN( MAX( z2d(:,:), 100._wp ) , 350._wp )
      !
   ...
   !===============================================================================================
   FUNCTION q_air_rh_sclr(prha, ptak, ppa)
      !!----------------------------------------------------------------------------------
   ...
   FUNCTION q_air_rh_vctr(prha, ptak, ppa)
      ...
      !! ** Author: L. Brodeau, june 2016 / AeroBulk (https://github.com/brodeau/aerobulk/)
      !!----------------------------------------------------------------------------------
      REAL(wp), DIMENSION(jpi,jpj)             :: q_air_rh_vctr
      ...
      DO jj=Njs0-1, Nje0+1
         DO ji=Nis0-1, Nie0+1
            ...
            q_air_rh_vctr(ji,jj) = ze*reps0/(ppa(ji,jj) - (1. - reps0)*ze)
         END DO
      END DO
      !
   END FUNCTION q_air_rh_vctr
   !===============================================================================================
