...
CONTAINS
   ...
   SUBROUTINE fld_get( sdjf, Kmm )
      ...
      IF( LEN_TRIM(sdjf%vcomp) > 0 ) THEN
         zsgn = 1._wp        ! geographical vectors -> sign change done later when rotating
      ELSE
         zsgn = sdjf%zsgn
      ENDIF
      !
   ...
   SUBROUTINE fld_def( sdjf, ldprev, ldnext )
      ...
      idaysec = NINT(rday)
      !
      IF( PRESENT(ldprev) ) THEN
         llprev = ldprev
      ELSE
         llprev = .FALSE.
      ENDIF
      IF( PRESENT(ldnext) ) THEN
         llnext = ldnext
      ELSE
         llnext = .FALSE.
      ENDIF
      ...
      IF    ( NINT(sdjf%freqh) == -12 ) THEN
         ireclast = 1    ! yearly mean: consider only 1 record
      ELSEIF( NINT(sdjf%freqh) ==  -1 ) THEN                                ! monthly mean:
         IF(     sdjf%clftyp      == 'monthly' ) THEN
            ireclast = 1    !  consider that the file has  1 record
         ELSE
            ireclast = 12   !  consider that the file has 12 record
         ENDIF
      ELSE                                                                  ! higher frequency mean (in hours)
         IF(     sdjf%clftyp      == 'monthly' ) THEN
            ireclast = NINT( 24. * REAL(nmonth_len(indexmt), wp) / sdjf%freqh )
         ELSEIF( sdjf%clftyp(1:4) == 'week'    ) THEN
            ireclast = NINT( 24. * 7.                            / sdjf%freqh )
         ELSEIF( sdjf%clftyp      == 'daily'   ) THEN
            ireclast = NINT( 24.                                 / sdjf%freqh )
         ELSE
            ireclast = NINT( 24. * REAL( nyear_len(indexyr), wp) / sdjf%freqh )
         ENDIF
      ENDIF
      ...
      ELSE                                                                       ! higher frequency mean (in hours)
         IF(     sdjf%clftyp      == 'monthly' ) THEN
            istart = nsec1jan000 + nmonth_beg(indexmt)
         ELSEIF( sdjf%clftyp(1:4) == 'week'    ) THEN
            istart = nsec1jan000 + isecwk
         ELSEIF( sdjf%clftyp      == 'daily'   ) THEN
            istart = nsec1jan000 + nmonth_beg(indexmt) + ( idy - 1 ) * idaysec
         ELSEIF( indexyr          == 0         ) THEN
            istart = nsec1jan000 - nyear_len( 0 ) * idaysec
         ELSEIF( indexyr          == 2         ) THEN
            istart = nsec1jan000 + nyear_len( 1 ) * idaysec
         ELSE
            istart = nsec1jan000
         ENDIF
