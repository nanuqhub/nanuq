...
!
MODULE sbcblk_coare_util

   USE par_kind, ONLY: wp
   USE par_oce,  ONLY: jpi, jpj, Nis0, Njs0, Nie0, Nje0
   USE phycst,   ONLY: rpi, grav, vkarmn, vkarmn2
   USE sbc_phy,  ONLY: visc_air, Ri_bulk
...
CONTAINS
   ...
   SUBROUTINE FIRST_GUESS_COARE_SCLR( zt, zu, psst, t_zt, pssq, q_zt, U_zu, pcharn, &
      ...
      !
      CHARACTER(len=40), PARAMETER :: crtnm = 'FIRST_GUESS_COARE_SCLR@sbcblk_coare_util.f90'
      !!----------------------------------------------------------------------------------
   ...
   SUBROUTINE FIRST_GUESS_COARE_VCTR( zt, zu, psst, t_zt, pssq, q_zt, U_zu, pcharn, &
      ...
      REAL(wp), INTENT(out), DIMENSION(jpi,jpj), OPTIONAL :: qz0    ! roughness length [m]
      !
      INTEGER  :: ji, jj
      REAL(wp) :: zz0
      !!----------------------------------------------------------------------------------
   ...
   FUNCTION psi_m_coare_vctr( pzeta )
      ...
      !! ** Author: L. Brodeau, June 2016 / AeroBulk (https://github.com/brodeau/aerobulk/)
      !!----------------------------------------------------------------------------------
      REAL(wp), DIMENSION(jpi,jpj)             :: psi_m_coare_vctr
   ...
   FUNCTION psi_h_coare_vctr( pzeta )
      ...
      !!         (https://github.com/brodeau/aerobulk/)
      !!----------------------------------------------------------------
      REAL(wp), DIMENSION(jpi,jpj)             :: psi_h_coare_vctr
      REAL(wp), DIMENSION(jpi,jpj), INTENT(in) :: pzeta
      !
      INTEGER  ::   ji, jj     ! dummy loop indices
