...
CONTAINS
   ...
   SUBROUTINE turb_coare3p0( kt, zt, zu, T_s, t_zt, q_s, q_zt, U_zu, l_use_cs, l_use_wl, &
      ...
      !!----------------------------------------------------------------------------------
      !IF( kt == nit000 ) CALL sbcblk_cs_wl_alloc(l_use_cs, l_use_wl)
      ...
      DO jit = 1, nbit
         ...
         IF( l_use_cs ) THEN
            ...
            PRINT *, 'LOLO fix me !!! Convert sbcblk_algo_coare3p0.F90 !!! to ji, jj like ECMWF !!!!'
            STOP
            ...
            T_s(:,:) = zsst(:,:) + dT_cs(:,:)*xmskt(:,:)
            IF( l_use_wl ) T_s(:,:) = T_s(:,:) + dT_wl(:,:)*xmskt(:,:)
         ...
         IF( l_use_wl ) THEN
            ...
            T_s(:,:) = zsst(:,:) + dT_wl(:,:)*xmskt(:,:)
            IF( l_use_cs ) T_s(:,:) = T_s(:,:) + dT_cs(:,:)*xmskt(:,:)
            q_s(:,:) = rdct_qsat_salt*q_sat(MAX(T_s(:,:), 200._wp), slp(:,:))
         ENDIF
