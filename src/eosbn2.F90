MODULE eosbn2
   ...
   !                  !! * Interface
   INTERFACE eos
      MODULE PROCEDURE eos_insitu_2d, eos_insitu_pot_2d
   END INTERFACE eos
   !
   INTERFACE eos_rab
      MODULE PROCEDURE rab_2d, rab_0d
   END INTERFACE eos_rab
   !
   !INTERFACE eos_fzp
   !   MODULE PROCEDURE eos_fzp_2d, eos_fzp_0d
   !END INTERFACE eos_fzp
   !
...
CONTAINS
   ...
   SUBROUTINE eos_insitu_2d( pts, pdep, prd )
      ...
      !!
      CALL eos_insitu_2d_t( pts, pdep, prd )
   END SUBROUTINE eos_insitu_2d
   !
   !
   SUBROUTINE eos_insitu_2d_t( pts, pdep, prd )
      !!----------------------------------------------------------------------
   ...
   SUBROUTINE eos_insitu_pot_2d( pts, prhop )
      ...
      !!
      CALL eos_insitu_pot_2d_t( pts, prhop )
   END SUBROUTINE eos_insitu_pot_2d


   SUBROUTINE eos_insitu_pot_2d_t( pts, prhop )
      !!----------------------------------------------------------------------
   ...
   SUBROUTINE rab_2d( pts, pdep, pab, Kmm )
      ...
      !!
      CALL rab_2d_t(pts, pdep, pab, Kmm)
   END SUBROUTINE rab_2d


   SUBROUTINE rab_2d_t( pts, pdep, pab, Kmm )
      !!----------------------------------------------------------------------
      !!                 ***  ROUTINE rab_2d  ***
      !!
   ...
   SUBROUTINE eos_fzp_2d( psal, ptf, pdep )
      ...
      !!
      CALL eos_fzp_2d_t( psal, ptf, pdep )
   END SUBROUTINE eos_fzp_2d


   SUBROUTINE  eos_fzp_2d_t( psal, ptf, pdep )
      !!----------------------------------------------------------------------
      !!                 ***  ROUTINE eos_fzp  ***
      !!
   ...
   SUBROUTINE  eos_fzp_2d_gpu( psal, ptf )
      ...
      CASE ( np_eos80 )                !==  PT,SP (UNESCO formulation)  ==!
         ...
         DO jj=Njs0-1, Nje0+1
            DO ji=Nis0-1, Nie0+1
               zs = psal(ji,jj)
               ptf(ji,jj) =  ( -0.0575_wp + 1.710523e-3_wp*SQRT(zs) - 2.154996e-4_wp*zs ) * zs
            END DO
         END DO
         !$acc end parallel loop
         !
   ...
   SUBROUTINE eos_init
      ...
      IF(lwp) WRITE(numout,*) '      ocean specific heat                 rcp   = ', rcp    , ' J/K/kg'
      IF(lwp) WRITE(numout,*) '      rho0 * rcp                       rho0_rcp = ', rho0_rcp, 'J/K/m^3'
      IF(lwp) WRITE(numout,*) '      1. / ( rho0 * rcp )           r1_rho0_rcp = ', r1_rho0_rcp
      !
      !$acc update device(  rho0, r1_rho0, rcp, r1_rcp, rho0_rcp, r1_rho0_rcp )
      !
