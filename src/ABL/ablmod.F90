MODULE ablmod
   ...
   !
   USE sbc_phy, ONLY  : theta_exner, pres_temp, z0_from_Cd, rho_air, cp_air, L_vap, rctv0
   !
...
CONTAINS
   ...
   SUBROUTINE abl_stp( kt, psst, pssu, pssv, pssq,            &     ! in
      ...
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj)        ::   psst       ! sea-surface temperature [Celsius]
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj)   ::   pssu       ! sea-surface u (U-point)
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj)   ::   pssv       ! sea-surface v (V-point)
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj)        ::   pssq       ! sea-surface humidity
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj,1:jpka) ::   pu_dta     ! large-scale windi
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj,1:jpka) ::   pv_dta     ! large-scale windj
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj,1:jpka) ::   pgu_dta    ! large-scale hpgi
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj,1:jpka) ::   pgv_dta    ! large-scale hpgj
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj,1:jpka) ::   pt_dta     ! large-scale pot. temp.
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj,1:jpka) ::   pq_dta     ! large-scale humidity
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj)        ::   pslp_dta   ! sea-level pressure
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj)        ::   pcd_du     ! Cd x Du (T-point)
      REAL(wp) , INTENT(inout), DIMENSION(jpi,jpj)        ::   psen       ! Ch x Du
      REAL(wp) , INTENT(inout), DIMENSION(jpi,jpj)        ::   pevp       ! Ce x Du
      REAL(wp) , INTENT(inout), DIMENSION(jpi,jpj)        ::   pwndm      ! ||uwnd - uoce||
      REAL(wp) , INTENT(  out), DIMENSION(jpi,jpj)        ::   plat       ! latent heat flux
      REAL(wp) , INTENT(  out), DIMENSION(jpi,jpj)   ::   ptaui      ! taux
      REAL(wp) , INTENT(  out), DIMENSION(jpi,jpj)   ::   ptauj      ! tauy
      REAL(wp) , INTENT(  out), DIMENSION(jpi,jpj)        ::   ptaum      ! ||tau||
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj)        ::   ptm_su     ! ice-surface temperature [K]
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj)   ::   pssu_ice   ! ice-surface u (U-point)
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj)   ::   pssv_ice   ! ice-surface v (V-point)
      REAL(wp) , INTENT(in   ), DIMENSION(jpi,jpj)        ::   pssq_ice   ! ice-surface humidity
      ...
      !
      REAL(wp), DIMENSION(jpi,jpj)         ::   zwnd_i, zwnd_j
      REAL(wp), DIMENSION(jpi,jpj)         ::   zsspt, ztabs, zpre
      REAL(wp), DIMENSION(jpi,2:jpka) ::   zCF
      REAL(wp), DIMENSION(jpi,jpj,1:jpka)  ::   zpdif
      REAL(wp), DIMENSION(jpi,jpj,1:jpka)  ::   zpabl, zpdta, zpgau, zpgav
      REAL(wp), DIMENSION(jpi,1:jpka) ::   z_elem_a, z_elem_b, z_elem_c
      !
      ...
      ENDIF
      !
      IF( kt == nit000 ) ALLOCATE ( ustar2(jpi,jpj) )
      IF( kt == nit000 ) ALLOCATE ( zrough(jpi,jpj) )
      ...
      DO jj=Njs0, Nje0
         DO ji=Nis0, Nie0
            ...
            !
            zrough(ji,jj) = z0_from_Cd( ght_abl(2), pCd_du(ji,jj) / MAX( pwndm(ji,jj), 0.5_wp ) ) ! #LB: z0_from_Cd is define in sbc_phy.F90...
         END DO
      END DO
      ...
      DO jj=Njs0, Nje0
         DO ji=Nis0, Nie0
            ...
            pevp( ji, jj ) = MAX( 0._wp,  zcff * pevp(ji,jj) * ( pssq(ji,jj)  - zhumi ) )
      ...
      DO jj=Njs0, Nje0
         DO ji=Nis0, Nie0
            ...
            ptaui_ice(ji,jj) = zztmp * ( u_abl(ji,jj,2,nt_a) - 0.5_wp * ( pssu_ice(ji,jj) + pssu_ice(ji-1,jj  ) ) )
            ptauj_ice(ji,jj) = zztmp * ( v_abl(ji,jj,2,nt_a) - 0.5_wp * ( pssv_ice(ji,jj) + pssv_ice(ji  ,jj-1) ) )
         END DO
      END DO
   ...
   SUBROUTINE abl_zdf_tke( )
      ...
      REAL(wp)                            ::   zwndi, zwndj
      REAL(wp), DIMENSION(jpi,1:jpka) ::   zsh2
      REAL(wp), DIMENSION(jpi,jpj ,1:jpka) ::   zbn2
      REAL(wp), DIMENSION(jpi,1:jpka) ::   zFC, zRH, zCF
      REAL(wp), DIMENSION(jpi,1:jpka) ::   z_elem_a
      REAL(wp), DIMENSION(jpi,1:jpka) ::   z_elem_b
      REAL(wp), DIMENSION(jpi,1:jpka) ::   z_elem_c
      LOGICAL                             ::   ln_Patankar    = .FALSE.
      LOGICAL                             ::   ln_dumpvar     = .FALSE.
      LOGICAL , DIMENSION(jpi)        ::   ln_foundl
      !
      tind  = nt_n
      ziRic = 1._wp / rn_Ric
