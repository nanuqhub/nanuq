...
CONTAINS
   ...
   SUBROUTINE stp_ctl( kt )
      ...
      !!                "nanuq.stat"  file = run statistics
      ...
      IF( kt == nit000 ) THEN
         ...
         IF( ll_wrtruns ) THEN
            clname = 'nanuq.stat'
            !                             ! open nanuq.stat     ascii file, done only by 1st subdomain
            CALL ctl_opn( numrun, clname, 'REPLACE', 'FORMATTED', 'SEQUENTIAL', -1, numout, lwp, narea )
            !                             ! open nanuq.stat.nc netcdf file, done only by 1st subdomain
      ...
      !
      llmsk(Nis0:Nie0,Njs0:Nje0) = xmskt(Nis0:Nie0,Njs0:Nje0) == 1._wp        ! test only the inner domain
      !
      ...
      !
      !                                   !==               get global extrema             ==!
      !                                   !==  done by all processes if writting nanuq.stat  ==!
      IF( ll_colruns ) THEN
